...
int solution[N]; // Array to store column numbers where queens are placed

/* A utility function to print solution and store column numbers */
...
{ 
    ...
    { 
        for (int j = 0; j < N; j++) {
            printf("%2d ", board[i][j]); 
            if (board[i][j] == 1)
                solution[i] = j; // Store column number if queen is placed
        }
        printf("\n"); 
    } 
} 
...
bool solveNQueensUtil(int board[N][N], int col, 
                      ...
                      bool rowLookup[N], bool slashCodeLookup[],  
...
{ 
    if (solveNQueens()) {
        printf("Column numbers where queens are placed:\n");
        for (int i = 0; i < N; i++) {
